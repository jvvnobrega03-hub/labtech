...
export function ContactForm() {
  ...
  if (status === "done") {
    return (
      <div className="contact-form contact-form--success rounded-[2rem] border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <h2 ref={confirmationHeading} tabIndex={-1} className="display mt-5 text-3xl">Mensagem validada</h2>
        <p className="mt-4 leading-7 text-slate-600">Este ambiente demonstrativo não armazena nem encaminha mensagens. A validação não confirma recebimento pela Labtech.</p>
        <button type="button" onClick={() => setStatus("idle")} className="button button-outline mt-7">Nova mensagem</button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={submit} aria-busy={status === "sending"} aria-describedby="contact-demo-notice" className="contact-form rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-9">
      ...
      <p id="contact-demo-notice" className="contact-form__demo-note mt-4 text-xs leading-5 text-slate-500">Nenhuma mensagem é persistida ou encaminhada neste ambiente.</p>
      ...
      <button type="submit" disabled={status === "sending"} className="button button-primary mt-7 disabled:opacity-50">{status === "sending" ? "Validando..." : "Validar mensagem"}</button>
    </form>
  );
}
