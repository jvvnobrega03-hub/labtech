...
import { companyExperienceLabel, siteConfig } from "@/lib/config";
      ...
      <div className="site-header__bar border-b border-white/10 bg-deep/95 backdrop-blur-xl">
        <div className="site-header__inner shell flex h-[88px] items-center gap-4">
          ...
          <form action="/catalogo" className="site-header__search relative ml-auto hidden w-full max-w-[255px] lg:block xl:hidden" role="search">
            ...
            <input id="header-search" name="q" type="search" className="site-header__search-input h-11 w-full border border-white/15 bg-white/[.06] pl-4 pr-11 text-xs text-white placeholder:text-white/45 focus:border-mint focus:bg-white/10 focus:outline-none" placeholder="Produto, categoria ou aplicação" />
            <button type="submit" className="site-header__search-button absolute right-1 top-1 grid size-9 place-items-center text-mint hover:bg-white/10" aria-label="Buscar"><SearchIcon className="size-4.5" /></button>
          </form>
          <HeaderActions />
        </div>
      </div>
...
export function Footer() {
  return (
    <footer className="technical-footer bg-deep text-emerald-50/70" data-technical-surface="footer">
      ...
      <div className="technical-footer__grid shell grid gap-12 py-16 md:grid-cols-[1.25fr_1fr_1fr_1fr]">
        <div className="technical-footer__brand">
          <Brand variant="light" />
        ...
        <div className="technical-footer__column">
      ...
      <div className="technical-footer__bottom border-t border-white/10">
